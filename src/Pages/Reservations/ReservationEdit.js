...
export default function ReservationUpdate() {
    ...
    useEffect(() => {
        if (reservationState.reservation) {
            const reservation = reservationState.reservation.find((cust) => cust.id === reservationId);
            if (reservation) {
                ...
                if (reservationDate) {
                    ...
                    const formattedDate = reservationDate.toLocaleString('sv-SE', options).replace(' ', 'T');
    ...
    const formatCurrency = (value) => `${value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")} VND`;
                                                ...
                                                <tbody>
                                                    {reservationDetailState.loading ? (
                                                        ...
                                                        groupedReservationDetails
                                                            .filter(
                                                                (item) =>
                                                                    selectedCategory === null || item.categories_id === selectedCategory
                                                            )
                                                            .map((item, index) => (
                                                                <tr key={`${item.product_id}-${index}`}>
                                                                    ...
                                                                    <td>
                                                                        ...
                                                                        <span
                                                                            ...
                                                                            onClick={() =>
                                                                                handleDeleteProduct(item.product_id, item.reservation_id)
                                                                            }
                                                                        >
                                                                            <i className="fas fa-trash-alt"></i> {/* Icon xóa */}
                                                                        </span>
                                                                    </td>
                                                                </tr>
                                                            ))
